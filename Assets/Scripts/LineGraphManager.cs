...
public class LineGraphManager : MonoBehaviour {
    ...
    public TextMesh midRangeMarker;
    public TextMesh highRangeMarker;
	...
	void Start(){
        //Lists of numbers that will later be the test data.
        //int[] testBaseMarbles = { 12, 11, 15, 8, 9, 14, 6, 10, 14, 12, 10, 11, 15, 8, 9, 14, 6, 10, 14, 12, 10, 11, 15, 8, 9, 14, 6, 10, 14, 12, 10, 11, 15, 8, 9, 14, 6, 10, 14, 12, 10, 11, 15, 8, 9, 14, 6, 10, 14, 12, 10 };
        //int[] testTestedMarbles = { 13, 11, 18, 1, 4, 5, 6, 7, 10, 12, 13, 11, 15, 8, 9, 14, 6, 10, 14, 12, 10, 11, 15, 8, 9, 14, 6, 10, 14, 12, 10, 11, 15, 8, 9, 14, 6, 10, 14, 12, 10, 11, 15, 8, 9, 14, 6, 10, 14, 12, 10 };
        //CreateGraph(testBaseMarbles, testTestedMarbles);
	}

    public void CreateHeadySteadyGraph(List<float> baselineData, List<float> testedData)
    {
        if(baselineData.Count != testedData.Count)
        {
            int numberToDelete = 0;
            if (baselineData.Count > testedData.Count)
            {
                numberToDelete = baselineData.Count - testedData.Count;
                for (int i = 0; i < numberToDelete; i++)
                    baselineData.RemoveAt(baselineData.Count - 1);
            }

            if (testedData.Count > baselineData.Count)
            {
                numberToDelete = testedData.Count - baselineData.Count;
                for (int i = 0; i < numberToDelete; i++)
                    testedData.RemoveAt(testedData.Count - 1);
            }
        }

        float[] baselineMarbles = baselineData.ToArray();
        for (int i = 0; i < baselineMarbles.Length; i++)
            baselineMarbles[i] = Convert.ToInt16(baselineMarbles[i] * 100);
            
        float[] testedMarbles = testedData.ToArray();
        for (int i = 0; i < testedMarbles.Length; i++)
            testedMarbles[i] = Convert.ToInt16(testedMarbles[i] * 100);


        print("sdasdadsadsa" + baselineMarbles.Length + "   " + testedMarbles.Length);

        CreateGraph(baselineMarbles, testedMarbles);
    }

    public void CreateGraph(float[] baseMarbles, float[] testedMarbles)
    {
        int index = baseMarbles.Length;
        for (int i = 0; i < index; i++)
        {
            GraphData gd = new GraphData();
            gd.marbles = baseMarbles[i];
            graphDataPlayer1.Add(gd);
            GraphData gd2 = new GraphData();
            gd2.marbles = testedMarbles[i];
